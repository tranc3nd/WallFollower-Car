...
void ReadADCMedianFilter(uint16_t *ain2, uint16_t *ain9, uint16_t *ain8);// This function samples AIN2 (PE1), AIN9 (PE4), AIN8 (PE5)
...
void steering(uint16_t ahead_dist,uint16_t right_dist, uint16_t left_dist){
  ...
  // 5. If both sensors see no obstacle within 30cm, go straight
	...
	if(enableMotor) {
			...
			if((left_dist>IR15CM)&&(right_dist>IR15CM)&&(ahead_dist>IR15CM)){//stop moving
			...
			else if((left_dist<IR70CM)&&(right_dist<IR70CM)&&(ahead_dist<IR60CM)){	//open area
				PWM_PB76_Duty(0,0);
				LED=Purple;
			}
			
			else if((left_dist>IR20CM)){	//right moving
				PWM_PB76_Duty((p_speed+FIFTY_PERCENT)*period,(FIFTN_PERCENT)*period);
				
			}
			else if((right_dist>IR20CM)){	// moving
				
				PWM_PB76_Duty((FIFTN_PERCENT)*period,(p_speed+FIFTY_PERCENT)*period);
				...
				if((right_dist-left_dist) > IR20CM){
					...
					PWM_PB76_Duty(((p_speed/2)+FIFTN_PERCENT)*period,(p_speed+THIRTY_PERCENT)*period);
			}
			else if(left_dist>right_dist){
				LED=Dark;
				if((left_dist-right_dist) > IR20CM){
					...
					PWM_PB76_Duty((p_speed+THIRTY_PERCENT)*period,((p_speed/2)+FIFTN_PERCENT)*period);
			}
			else {//if((right_dist>IR60CM)&&(left_dist>IR60CM)){
				LED=Dark;
				PWM_PB76_Duty((p_speed+FOURTY_PERCENT)*period,(p_speed+FOURTY_PERCENT)*period);
			}
	}else {
		LED = 0;
...
void GPIOPortF_Handler(void){ // called on touch of either SW1 or SW2
  if(GPIO_PORTF_RIS_R&0x01) {  // SW2 touched - changes wheel direction
		Delay();
  ...
  if(GPIO_PORTF_RIS_R&0x10){  // SW1 touch - increases increments speed flag
		Delay();
